...
namespace CleanCacheTool
{
    public static class FolderUtil
    {
        ...
        public static long GetFolderSpaceSize(string folder)
        {
            ...
            //文件占用空间
            var filesSize = directoryInfo.GetFiles().Sum(i => i.Length);
            totalSpaceSize += filesSize;

            //子目录占用空间
            var directoriesSize = directoryInfo.GetDirectories().Sum(i => GetFolderSpaceSize(i.FullName));
            totalSpaceSize += directoriesSize;
